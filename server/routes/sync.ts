...
syncRouter.get('/pull', async (req: Request, res: Response) => {
  try {
    ...
    const [items, parties, invoices, estimates, paymentsIn, purchaseOrders, purchaseBills, paymentsOut, expenses, purchaseReturns, saleReturns, cashAccounts, cashTransactions, rawLocations, rawItemLocations, rawStockTransfers, storeWarehouseAccess] = await Promise.all([
      ...
      Invoice.findAll({ where: { tenantId }, include: [{ model: InvoiceItem, as: 'items' }] }),
      Estimate.findAll({ where: { tenantId }, include: [{ model: EstimateItem, as: 'items' }] }),
      PaymentIn.findAll({ where: { tenantId } }),
      PurchaseOrder.findAll({ where: { tenantId }, include: [{ model: PurchaseOrderItem, as: 'items' }] }),
      PurchaseBill.findAll({ where: { tenantId }, include: [{ model: PurchaseBillItem, as: 'items' }] }),
      PaymentOut.findAll({ where: { tenantId } }),
      Expense.findAll({ where: { tenantId } }),
      PurchaseReturn.findAll({ where: { tenantId }, include: [{ model: PurchaseReturnItem, as: 'items' }] }),
      SaleReturn.findAll({ where: { tenantId }, include: [{ model: SaleReturnItem, as: 'items' }] }),
