...
export const ReportsScreen: React.FC<ReportsScreenProps> = ({
  ...
  // Filter Invoices for Sale Report with Strict Tenant Isolation
  const filteredInvoices = useMemo(() => {
    return safeInvoices.filter(inv => {
      if (!inv) return false;

      // Normalize Tenant ID for robust multi-tenant isolation
      const invTenant = inv.tenantId || 'default-tenant';

      // Firm / Tenant Isolation Filter
  ...
  // Previous Month Sales Total for Growth % Calculation (Tenant Isolated)
  ...
  // Export Filtered Table to CSV / Excel File
  ...
  // Print Entire Report Summary Page
        ...
        {activeTab === 'sale' ? (
          <>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-3.5 shadow-sm flex flex-wrap items-center justify-between gap-3 text-xs">
              ...
              {/* Firm Selector */}
              <div className="flex items-center gap-2">
                <span className="text-slate-500 font-bold">Firm:</span>
                <select
                  value={selectedFirm}
                  onChange={e => setSelectedFirm(e.target.value)}
                  aria-label="Select firm or tenant"
                  className="h-9 px-3 bg-slate-50 border border-slate-200 rounded-lg text-slate-800 font-bold outline-none focus:border-blue-500 cursor-pointer min-w-[140px]"
                >
            ...
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm flex flex-col overflow-hidden">
              {/* Transactions Header & Utility Toolbar */}
          ...
          /* Placeholder View for other report tabs */
